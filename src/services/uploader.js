...
export async function uploadEpisode(episode, seriesInfo, onProgress = null) {
  ...
  try {
    ...
    // Upload video
    const dailymotionVideoId = await client.uploadAndPublishVideo(
      path,
      videoTitle,
      videoDescription,
      progressCallback
    ...
    const result = await client.query(
      'SELECT retry_count FROM upload_schedule WHERE episode_id = $1',
      [episodeId]
    );
    client.release();
    ...
    if (retryCount >= 2) { // Already tried 3 times (0, 1, 2)
...
export async function addSeriesToSchedule(seriesId) {
  ...
  try {
    ...
    const totalEpisodesResult = await client.query(
    ...
    for (const episode of episodesResult.rows) {
      ...
      const existing = await client.query(
        'SELECT id FROM upload_schedule WHERE episode_id = $1',
        [episode.id]
      );
