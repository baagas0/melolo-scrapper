...
import { searchAndSaveBooks, getBooks, getUnscrapedBooks, markBookAsScraped, deleteBook } from "./services/books.js";
import { scrapeSeries, getEpisodesToDownload } from "./services/scraper.js";
import { downloadSeriesEpisodes, downloadAllSeries } from "./services/downloader.js";
import { getQueueStatus, clearQueue, retryFailed } from "./services/downloadQueue.js";
import { addSeriesToSchedule, getUploadSchedule, removeSeriesFromSchedule, testDailymotionConnection, testUploadEpisode, getEpisodeForTest } from "./services/uploader.js";
...
app.post("/api/scrape", async (req, res) => {
  try {
    ...
    if (download !== false) {
      ...
      downloadedPaths = await downloadSeriesEpisodes(scrapedData.seriesId, outputDir || "./video", concurrency || 1, progressCallback);
...
app.get("/api/stats", async (req, res) => {
  try {
    ...
    const [booksResult, scrapedResult, seriesResult, episodesResult] = await Promise.all([client.query("SELECT COUNT(*) as count FROM books"), client.query("SELECT COUNT(*) as count FROM books WHERE scraped = true"), client.query("SELECT COUNT(*) as count FROM series"), client.query("SELECT COUNT(*) as count FROM episodes")]);
...
app.post("/api/download/all", async (req, res) => {
  try {
    const { concurrency, outputDir } = req.body;

    broadcastMessage({
      type: "download_all_start",
      data: { message: "Starting sequential download of all series" },
    });

    // Start background process
    downloadAllSeries(outputDir || "./video", concurrency || 1, (type, data) => {
      // Map to existing message types for compatibility where possible
      if (type === "episode_progress") {
        broadcastMessage({
          type: "download_progress",
          data: {
    ...
    res.json({
      success: true,
      message: "Started downloading all series",
    });
  } catch (error) {
    console.error("Download all error:", error);
    res.status(500).json({
...
app.get("/api/upload/test/episode-info", async (req, res) => {
  try {
    ...
    const episode = await getEpisodeForTest(parseInt(seriesId), episodeIndex ? parseInt(episodeIndex) : 1);
  ...
  for (const book of books) {
    try {
      ...
      if (download) {
        ...
        await downloadSeriesEpisodes(scrapedData.seriesId, outputDir, concurrency, progressCallback);
      }
